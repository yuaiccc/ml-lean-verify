...
```
lake build           →  2974 个编译任务  →  零错误零 sorry 零警告
...
│   ├── train_robust_model.py  # IBP 认证鲁棒性训练
│   ├── verify_robustness.py   # Z3 SMT L∞ 鲁棒性验证
│   ├── verify_robust_model.py # IBP 模型 Z3 验证
│   ├── pgd_attack.py          # PGD 对抗攻击（互补 SMT）
│   ├── visualize.py           # 结果可视化
│   ├── verification_results.json      # 原始模型 SMT 结果
│   ├── verification_results_robust.json # IBP 模型 SMT 结果
│   ├── pgd_results.json       # PGD 攻击结果
│   ├── verification_heatmap.png   # SMT vs PGD 对比热力图
│   └── adversarial_examples.png   # 对抗样本可视化
├── scripts/                   # CI 与本地验证脚本
│   ├── verify_all.sh          # 一键运行全部验证
│   └── check_sorry.sh         # 扫描 sorry
...
| `softmax_sum_eq_one` | **归一性**：softmax 分量和为 1 |
| `attention_weights_nonneg` | attention 权重非负时输出非负 |
...
| `routedExperts_switch_one` | Switch 模式只激活 1 个专家 |
| `routedExperts_moe_k` | MoE 模式激活恰好 topK 个专家 |
...
| `layerNorm_denom_pos` | LayerNorm 缩放因子 > 0（非空向量 + eps > 0） |
| `rmsNorm_denom_pos` | RMSNorm 缩放因子 > 0（非空向量 + eps > 0） |
| `layerNorm_zero_mean` | **零均值**：LayerNorm 归一化后均值为 0 |

### Position Encoding（4 条）

| 定理 | 含义 |
|------|------|
| `ropeAngle_correct` | **旋转角公式**：`ropeAngle = pos / base^(2i/d)` |
| `yarnAngle_scaled` | **YaRN = RoPE(base*scale)**：角度等价于放大 base |
| `sinusoidalPE_bounded` | 正弦 PE 分量在 [-1, 1] 内 |
| `rope_freq_decreasing` | **频率递减**：RoPE 低维度对旋转更快 |
...
使用 Interval Bound Propagation (IBP) 重新训练网络，然后再次用 Z3 验证：
...
IBP 训练将 Z3 可证明的鲁棒样本数从 2 个提升到 6 个（**3 倍提升**），且没有产生任何 UNSAFE 结果。即使 IBP 的区间界本身过于宽松（认证准确率接近 0），IBP 训练仍然有效改善了决策边界的鲁棒性。

```bash
cd smt_verify
python3 train_robust_model.py     # IBP 训练
python3 verify_robust_model.py    # Z3 验证 IBP 模型
```
...
`Basic.lean` 和 `Safety.lean` 中声明了 35 条 `axiom`，对应 IEEE 754 binary64 的标准性质，对所有非 NaN 值成立。

## CI 自动化

### GitHub Actions

每次 push 到 main 或提交 PR 时自动触发两个并行 Job：

| Job | 内容 |
|-----|------|
| `lean-verify` | Lean 4 构建 + sorry 检查 + 差分测试 |
| `smt-verify` | Z3 SMT 验证 + PGD 攻击 |

### 本地一键验证

```bash
# 运行全部验证（Lean + 差分测试 + SMT + PGD）
...
# 跳过 SMT 和 PGD
./scripts/verify_all.sh --skip-smt --skip-pgd
```
...
| ml-lean-verify | AI 架构 | Transformer 组件数学性质 + MNIST 网络鲁棒性 | 25 条 + 30 SMT 查询 |
| [contract-lean-verify](https://github.com/yuaiccc/contract-lean-verify) | 区块链 | ERC-20 代币合约安全不变量 | 14 条 + 15 引理 |

三个项目展示了同一套形式化验证方法在不同领域的应用：从「交易信号守卫」到「Transformer 架构安全」到「智能合约不变量」。
